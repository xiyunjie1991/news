...
export default {
  getNews ({ commit, state }, payload) {
    return new Promise(resolve => {
      ...
      if (payload.type) {
        let url = urlForGetNews(payload.type, 0, 9); //'http://47.111.24.228:10080/app/user/message/info_finance?start=1&end=8';
        debugger;
        axios.get(url)
          .then(res => {
            debugger;
            let contents = JSON.parse(res.data.content);
            console.log("first img list count:\n" + contents[0].image_list.length);
            if (payload.channel === "home") {
  ...
  getMoreNews ({commit, state}, payload) {
    return new Promise(resolve => {
      ...
      if (start < maxNewsCount) {
        let url = urlForGetNews(payload.type, start, start + 9);
        axios.get(url)
          .then(res => {
            let contents = JSON.parse(res.data.content);
            commit(type.GET_NEWSLIST, {
